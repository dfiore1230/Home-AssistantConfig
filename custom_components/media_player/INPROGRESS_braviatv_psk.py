...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_APP_FILTER), default=[]): vol.All(cv.ensure_list, [cv.string])
})
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    app_filter = config.get(CONF_APP_FILTER)

    if host is None:
        _LOGGER.error("No TV IP address found in configuration file")
        return

    add_devices([BraviaTVDevice(host, psk, mac, name, amp, android, source_filter, app_filter)])

class BraviaTVDevice(MediaPlayerDevice):
    """Representation of a Sony Bravia TV."""

    def __init__(self, host, psk, mac, name, amp, android, source_filter, app_filter):
        ...
        _LOGGER.debug("Set up Sony Bravia TV with IP: " + host + " - PSK: " + psk + " - MAC: " + mac + " - APP_LIST" + app_list)
    ...
    def select_app(self, app):
        ...
        if app in self._app_list:
            uri + self.app_list[app]
            self._braviarc.openapp(uri)
